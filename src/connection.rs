...
pub struct Connection {
    pub(crate) zbus: zbus::blocking::Connection,
    pub(crate) path: OwnedObjectPath,
}

crate::zproxy_pathed!(
    Connection,
    crate::raw::settings_connection::SettingsConnectionProxyBlocking<'_>
);
...
impl Connection {
    ...
    pub fn update(&self, properties: HashMap<&str, HashMap<&str, Value<'_>>>) -> Result<(), Error> {
        self.raw()?.update(properties).map_err(Error::ZBus)
    }
    ...
    pub fn update_in_memory(
        ...
        self.raw()?.update_unsaved(properties).map_err(Error::ZBus)
    }
    ...
    pub fn settings(&self) -> Result<HashMap<String, HashMap<String, OwnedValue>>, Error> {
        self.raw()?.get_settings().map_err(Error::ZBus)
    }
    ...
    pub fn secrets(&self) -> Result<HashMap<String, HashMap<String, OwnedValue>>, Error> {
        self.secrets_for_setting("")
    }
    ...
    pub fn is_saved(&self) -> Result<bool, Error> {
        self.raw()?.unsaved().map(|flag| !flag).map_err(Error::ZBus)
    }

    /// Additional flags of the connection profile.
    pub fn flags(&self) -> Result<ConnectionFlags, Error> {
    ...
    pub fn filename(&self) -> Result<PathBuf, Error> {
        self.raw()?
            .filename()
            .map(|path| path.into())
            .map_err(Error::ZBus)
